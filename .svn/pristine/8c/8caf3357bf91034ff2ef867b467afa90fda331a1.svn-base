...
public class SoundPlayer
{
     ...
     * @throws IllegalArgumentException if there is a problem reading from the
     *             sound file.
     */
     ...
     * keeps looping until it is later stopped by calling the stop() method.
    ...
    /**
     * Plays the clip with the given file name in a loop. The clip loops until
     * it has played the specified number of times, or until it is later stopped
     * by calling the stop() method. This function returns instantly, without
     * waiting for the clip to finish looping.
     *
     * @param theFilename The name of the file to play.
     ...
     * will resume from where it was paused. Calling this method does not resume
     * a thread that is suspended on a playAndWait() or a loopAndWait().
     * 
     * If stop() is called on a paused clip, it will reset to the beginning of
     * the clip for the next play.
     * 
     ...
     *                the sound file.
     */
    public Clip stop(final String theFilename) throws IllegalArgumentException
    {
        final Clip clip = getClip(theFilename);
        stopClip(clip);
     ...
     *                the sound file.
     */
    public Clip preLoad(final String theFilename) throws IllegalArgumentException
    {
    ...
    private Clip getClip(final String theFilename) throws IllegalArgumentException
    {
        ...
        else
        {
            ...
            {
                throw new IllegalArgumentException("I/O error while reading file: \""
                                                   + theFilename + "\" ", ioe);
            }
        }
